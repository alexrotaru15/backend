...
async function generateUserProfile(userId, messages, retryCount = 0) {
	try {
		...
		// Create a prompt for the LLM - modified for doctorul's channel context
		const prompt = `Analizează aceste mesaje de chat Twitch din canalul lui Doctorul și generează un profil de utilizator amuzant și captivant în format JSON.
...
Pune accent pe referințe medicale, sfaturi de sănătate sau comentarii despre sistemul medical, dacă există.
...
async function generateAllUserProfiles() {
	try {
		...
		const doctorulChannel = await Channel.findOne({
			where: { id: "272573477" }, // Using the specific channel ID for doctorul
		});
			...
			include: [
				{
					...
					where: { channelId: doctorulChannel.id },
					attributes: [],
				},
				{
					model: User,
